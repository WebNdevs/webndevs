...
type SinglePageCategoryModel = {
  id: string;
  name: string;
  slug: string;
};

const DEFAULT_CATEGORIES: SinglePageCategoryModel[] = [
  { id: "cat-1", name: "General", slug: "/general" },
  { id: "cat-2", name: "Services", slug: "/services" },
  { id: "cat-3", name: "Solutions", slug: "/solutions" },
];

type SinglePageItemModel = {
  id: number;
  sort_order: number;
  is_featured: boolean;
  is_active: boolean;
...
type SinglePageSectionModel = {
  id: number;
  singlepage_page_id: number;
  section_key: string;
  section_type: string;
  is_visible: boolean;
  sort_order: number;
  data: Record<string, unknown>;
  items?: SinglePageItemModel[];
  updated_at: string | null;
};

type SinglePagePageModel = {
  id: number;
  title: string;
  slug: string;
  status: PageStatus;
  ...
  updated_at: string | null;
  sections: SinglePageSectionModel[];
...
export function SinglePageModule() {
  const [pages, setPages] = useState<SinglePagePageModel[]>([]);
  const [selectedPageSlug, setSelectedPageSlug] = useState<string>("");
  const [selectedPage, setSelectedPage] = useState<SinglePagePageModel | null>(null);
  ...
  const [categories, setCategories] = useState<SinglePageCategoryModel[]>(() => {
    try {
      const saved = localStorage.getItem("single_page_categories");
      if (saved) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("single_page_categories", JSON.stringify(categories));
    } catch {
      // ignore
    }
  }, [categories]);
  ...
  const filteredPages = useMemo(() => {
    ...
    return pages.filter((p) => (p.category_slug || "/general") === selectedCategorySlug);
  }, [pages, selectedCategorySlug]);

  useEffect(() => {
    if (filteredPages.length > 0) {
      const exists = filteredPages.some((p) => p.slug === selectedPageSlug);
      if (!exists) {
        setSelectedPageSlug(filteredPages[0].slug);
      }
    } else {
      setSelectedPageSlug("");
      setSelectedPage(null);
    }
  }, [filteredPages, selectedCategorySlug]);
  ...
  const openEditCategory = (cat: SinglePageCategoryModel) => {
  ...
  const handleSaveCategory = () => {
    let formattedSlug = categoryForm.slug.trim();
    ...
    if (isEditingCategory && editingCategoryId) {
      setCategories((prev) =>
        prev.map((c) => (c.id === editingCategoryId ? { ...c, name: categoryForm.name.trim(), slug: formattedSlug } : c))
      );
      const currentCat = categories.find((c) => c.id === editingCategoryId);
      if (currentCat && selectedCategorySlug === currentCat.slug) {
        setSelectedCategorySlug(formattedSlug);
      }
      setSuccessText(`Category "${categoryForm.name.trim()}" updated.`);
    } else {
      const newCat: SinglePageCategoryModel = {
        id: `cat-${Date.now()}`,
      ...
      setSelectedCategorySlug(formattedSlug);
      setSuccessText(`Category "${categoryForm.name.trim()}" created.`);
    }
    setShowCategoryModal(false);
  };

  const triggerDeleteCategory = (cat: SinglePageCategoryModel) => {
    setDeleteConfirm({
      isOpen: true,
      title: "Delete Category",
      message: `Are you sure you want to delete category "${cat.name}" (${cat.slug})?`,
      onConfirm: () => {
        ...
        setShowCategoryModal(false);
        setSuccessText(`Category "${cat.name}" deleted.`);
      },
    });
  };
  ...
  const authHeaders = useMemo(() => {
    return {
      "SinglePage-Type": "application/json",
  ...
  async function loadPages() {
    ...
    setErrorText("");
    try {
      const data = await requestJson<SinglePagePageModel[]>("/singlepage-pages");
      setPages((prev) => {
        const catMap = new Map(prev.map((p) => [p.slug, p.category_slug]));
        return data.map((p) => ({
          ...p,
          category_slug: p.category_slug || catMap.get(p.slug) || "/general",
        }));
      });
  ...
  async function loadPage(slug: string) {
    ...
    try {
      ...
      setErrorText(err instanceof Error ? err.message : "Failed to load page singlepage.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  const openCreatePage = () => {
    setPageForm({
      title: "",
      slug: "",
      category_slug: selectedCategorySlug !== "all" ? selectedCategorySlug : (categories[0]?.slug || "/general"),
      status: "draft",
      seo_title: "",
      seo_description: "",
      meta_keywords: "",
  ...
  const openEditPage = () => {
    ...
    setPageForm({
      ...
      category_slug: selectedPage.category_slug || categories[0]?.slug || "/general",
      ...
      if (isEditingPage && selectedPage) {
        await requestJson(`/singlepage-pages/${encodeURIComponent(selectedPage.slug)}`, {
          ...
          body: JSON.stringify({
            ...
            slug: pageForm.slug,
            category_slug: pageForm.category_slug,
        ...
        setPages((prev) =>
          prev.map((p) => (p.slug === selectedPage.slug ? { ...p, category_slug: pageForm.category_slug } : p))
        );
        setSelectedPage((prev) => (prev ? { ...prev, category_slug: pageForm.category_slug } : null));
        setSuccessText("Page updated successfully.");
      } else {
        const created = await requestJson<SinglePagePageModel>("/singlepage-pages", {
          method: "POST",
          body: JSON.stringify({
            title: pageForm.title,
            slug: pageForm.slug,
        ...
        created.category_slug = pageForm.category_slug;
  ...
  const triggerDeletePage = () => {
    ...
    setDeleteConfirm({
      isOpen: true,
      title: "Delete Page",
      message: `Are you sure you want to delete page "${selectedPage.title}"? This will permanently delete the page and all its singlepage sections.`,
      onConfirm: async () => {
        setDeleteConfirm((prev) => ({ ...prev, isOpen: false }));
        setIsSaving(true);
        setErrorText("");
  ...
  async function saveManagedSection(secKey: SectionKey) {
    ...
    try {
      ...
      const deletedPromises = deletedItemIds.map(async (id) => {
        if (id > 0) {
          await requestJson(`/singlepage-sections/${sectionId}/items/${id}`, {
            method: "DELETE",
          });
        }
      });
  ...
  return (
    <div className="p-2xl flex flex-col gap-xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-md">
        <div>
          <h1 className="text-title text-text-primary">SinglePage Pages</h1>
          <p className="text-label-sm text-text-secondary mt-xs">Manage your website pages and sections singlepage dynamically.</p>
        </div>
      ...
      {/* Category Tabs & Page Pills */}
      <div className="bg-surface-bg rounded-corner-lg p-xl flex flex-col gap-lg border border-border-primary">
        {/* Category Header */}
        <div className="flex items-center justify-between flex-wrap gap-md border-b border-border-secondary pb-md">
          <div className="flex items-center gap-xs">
            <Folder size={16} className="text-text-primary" />
            <span className="text-label-sm font-bold text-text-primary">Categories</span>
          </div>
          <div className="flex items-center gap-xs">
            <CustomButton variant="neutral" size="small" iconStart={<FolderPlus size={14} />} onClick={openCreateCategory}>
              Add Category
            </CustomButton>
          </div>
        </div>
        ...
        <div className="flex items-center gap-sm flex-wrap">
          ...
          />
          {categories.map((cat) => (
            <div key={cat.id} className="flex items-center gap-xs">
              <Pill
              ...
              {selectedCategorySlug === cat.slug && (
                <div className="flex items-center gap-[2px]">
                  <button
                    ...
                    className="p-1 hover:bg-bg-faint rounded text-text-tertiary hover:text-text-primary transition-colors cursor-pointer"
                  >
                    <Pencil size={13} />
                  </button>
                  <button
                    ...
                    className="p-1 hover:bg-bg-faint rounded text-text-tertiary hover:text-danger transition-colors cursor-pointer"
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
        ...
        {/* Page Pills */}
        <div className="pt-md border-t border-border-secondary flex flex-col gap-sm">
          <span className="text-label-xs font-semibold text-text-secondary">Pages in Category ({filteredPages.length})</span>
          <div className="flex flex-wrap gap-sm">
            {filteredPages.length === 0 ? (
              <p className="text-label-xs text-text-tertiary italic">No pages in this category.</p>
            ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 backdrop-blur-sm p-4">
          <div className="bg-surface-bg border border-border-primary rounded-xl max-w-xl w-full p-6 shadow-2xl flex flex-col gap-xl">
            ...
            <div className="flex flex-col gap-lg overflow-y-auto max-h-[50vh] pr-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
                <div>
                  <label className="mb-xs block text-label-sm font-medium text-text-secondary">Category</label>
                  <select
                    className="w-full rounded-lg border border-border-primary bg-bg-faint px-md py-sm text-text-primary select-none cursor-pointer"
                    value={pageForm.category_slug}
                    onChange={(e) => setPageForm((prev) => ({ ...prev, category_slug: e.target.value }))}
                  >
                    {categories.map((cat) => (
      ...
      {showCategoryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-surface-bg border border-border-primary rounded-xl max-w-md w-full p-6 shadow-2xl flex flex-col gap-xl">
            <div className="border-b border-border-secondary pb-md flex justify-between items-center">
              <div>
                <h3 className="text-label font-bold text-text-primary">
                ...
                <p className="text-label-sm text-text-secondary mt-1">
                  Specify category name and URL slug (e.g. /services).
                </p>
              </div>
              <button
                className="text-text-secondary hover:text-text-primary text-xl font-bold p-xs cursor-pointer"
            ...
            <div className="flex flex-col gap-lg">
              ...
              <InputField
                ...
                onChange={(v) => setCategoryForm((prev) => ({ ...prev, slug: v }))}
              />
            </div>
            ...
            <div className="flex items-center justify-between pt-lg border-t border-border-secondary">
              {isEditingCategory && editingCategoryId ? (
                <CustomButton
                  ...
                  onClick={() => {
                    ...
                    if (cat) triggerDeleteCategory(cat);
                  }}
                >
                  Delete Category
                </CustomButton>
              ...
              <div className="flex items-center gap-md">
                <CustomButton variant="primary" onClick={() => setShowCategoryModal(false)}>
                  Cancel
                </CustomButton>
                <CustomButton
                  variant="primary"
