...
export function ContentModule() {
  const [pages, setPages] = useState<ContentPageModel[]>([]);
  ...
  async function loadPages() {
    ...
    setErrorText("");
    try {
      const data = await requestJson<ContentPageModel[]>("/content-pages");
      setPages(data);
  ...
  const handleSavePage = async () => {
    ...
    try {
      ...
      if (isEditingPage && selectedPage) {
        ...
        const created = await requestJson<ContentPageModel>("/content-pages", {
          method: "POST",
          body: JSON.stringify({
            title: pageForm.title,
            slug: pageForm.slug,
    ...
    try {
      ...
      const savePromises = sec.items.map(async (item) => {
        ...
        if (item.id && item.id > 0) {
          await requestJson(`/content-sections/${sectionId}/items/${item.id}`, {
            method: "PUT",
            body: JSON.stringify(itemPayload),
          });
        } else {
          await requestJson(`/content-sections/${sectionId}/items`, {
            method: "POST",
            body: JSON.stringify(itemPayload),
          });
        }
    ...
    <div className="p-2xl flex flex-col gap-xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-md">
        <div>
          ...
          <p className="text-label-sm text-text-secondary mt-xs">Manage your website pages and sections content dynamically.</p>
        </div>
                ...
                <div className="flex flex-col gap-lg">
                  <Card title="Page Publish Control">
                    <p className="text-label-sm text-text-secondary mb-md">
                      Clear the frontend cache to publish all saved content changes live.
                    </p>
