...
type ArticleSectionModel = {
  id: number;
  article_page_id: number;
  section_key: string;
  section_type: string;
  is_visible: boolean;
  sort_order: number;
  data: Record<string, unknown>;
  items?: ArticleItemModel[];
  updated_at: string | null;
};
...
export function ArticleModule() {
  const [pages, setPages] = useState<ArticlePageModel[]>([]);
  ...
  async function loadPages() {
    ...
    setErrorText("");
    try {
      const data = await requestJson<ArticlePageModel[]>("/article-pages");
      setPages(data);
  ...
  const handleSavePage = async () => {
    ...
    try {
      ...
      if (isEditingPage && selectedPage) {
        ...
        const created = await requestJson<ArticlePageModel>("/article-pages", {
          method: "POST",
          body: JSON.stringify({
            title: pageForm.title,
            slug: pageForm.slug,
  ...
  const triggerDeletePage = () => {
    ...
    setDeleteConfirm({
      isOpen: true,
      title: "Delete Page",
      message: `Are you sure you want to delete page "${selectedPage.title}"? This will permanently delete the page and all its content sections.`,
      onConfirm: async () => {
  ...
  return (
    <div className="p-2xl flex flex-col gap-xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-md">
        <div>
          <h1 className="text-title text-text-primary">Articles</h1>
          <p className="text-label-sm text-text-secondary mt-xs">Manage your website articles and sections content dynamically.</p>
        </div>
        ...
        <>
          ...
          {isPublishingWorkflowActive ? (
            <div className="flex flex-col gap-xl">
              <h2 className="text-label text-text-primary">Publishing Articles</h2>
              
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-xl">
                {/* Save Draft & Publish */}
                <div className="flex flex-col gap-lg">
                  <Card title="Article Publish Control">
                ...
                <Card>
                  ...
                  {activeTab === "hero" ? (
                    ...
                    <div className="border-t border-border-secondary pt-md mt-md flex flex-col gap-lg bg-bg-faint p-lg rounded-corner-lg border border-border-primary">
                      <p className="font-semibold text-label-sm text-text-primary">Content Section</p>
                    </div>
                  ) : activeTab === "cta" ? (
                  ...
                  {/* Section Items */}
                  {!["hero", "header", "cta"].includes(activeTab) && (
                      ...
                      <div className="flex flex-col gap-lg">
                        ...
                        {activeManagedSection.items.map((item, itemIndex) => (
                          ...
                          >
                            ...
                            {activeManagedSection.section_key === "content" ? (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
                                <InputField
                                  label="Title"
                                ...
                                <InputField
                                  ...
                                  value={item.author || ""}
                                  onChange={(v) => updateManagedSectionItem(itemIndex, "author", v)}
                                />
                                <InputField
                                  label="Slug"
                                  placeholder="rise-of-ai-agents"
                                  value={item.slug || ""}
                                  onChange={(v) => updateManagedSectionItem(itemIndex, "slug", v)}
                                />
                                <InputField
                                  label="Date"
                                  placeholder="06-10-2026"
                                  value={item.date || ""}
                                  onChange={(v) => updateManagedSectionItem(itemIndex, "date", v)}
                                />
                                <InputField 
                                  label="Image URL"
                                ...
                                <TextareaField
                                  label="Content"
                                  placeholder="Artificial intelligence has become a core part of modern development workflows."
                                  value={item.content || ""}
                                  rows={4}
                                  onChange={(v) => updateManagedSectionItem(itemIndex, "content", v)}
                                />
                                <TextareaField
                                  label="Excerpt / Summary"
                                  placeholder="Discover how AI copilots, automation agents"
                                  value={item.excerpt || ""}
                                  rows={4}
                                  onChange={(v) => updateManagedSectionItem(itemIndex, "excerpt", v)}
                                />
                              </div>
                            ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 backdrop-blur-sm p-4">
          <div className="bg-surface-bg border border-border-primary rounded-xl max-w-xl w-full p-6 shadow-2xl flex flex-col gap-xl">
            <div className="border-b border-border-secondary pb-md flex justify-between items-center">
              <div>
                <h3 className="text-label font-bold text-text-primary">
                  {isEditingPage ? "Edit Article Details" : "Create New Article"}
                </h3>
                <p className="text-label-sm text-text-secondary mt-1">
                  Define the article URL (slug), name, SEO configurations, and hero info.
                </p>
              </div>
              <button
            ...
            <div className="flex flex-col gap-lg overflow-y-auto max-h-[50vh] pr-sm">
              ...
              <TextareaField
                label="Hero Description"
                placeholder="Explain the article focus..."
            ...
            <div className="flex items-center justify-end gap-md pt-lg border-t border-border-secondary">
              ...
              >
                {isSaving ? "Saving..." : isEditingPage ? "Save Changes" : "Create Article"}
              </CustomButton>
            </div>
          </div>
        </div>
